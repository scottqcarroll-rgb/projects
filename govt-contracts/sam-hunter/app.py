...
def score_contract(c):
    ...
    city  = c.get("placeOfPerformance", {}).get("city", {}).get("name", "") if isinstance(c.get("placeOfPerformance"), dict) else ""
    state = c.get("placeOfPerformance", {}).get("state", {}).get("code", "") if isinstance(c.get("placeOfPerformance"), dict) else ""
...
def api_search():
    ...
    try:
        ...
        return jsonify({"error": "Invalid JSON response from SAM.gov"}), 500
    except requests.exceptions.RequestException as e:
        return jsonify({"error": f"SAM.gov network error: {e}"}), 500
